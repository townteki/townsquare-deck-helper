pipeline {
    ...
    post {
        success {
            script {
                ...
                if(previousBuild && !previousBuild.result.toString().equals('SUCCESS')) {
                    mail body: "Hello ${GIT_NAME},\n\nI'm please to report that the throneteki-deck-helper build is now working again.  Please find details of the build here:\n\n${env.BUILD_URL}\n\nKind regards,\nThe Iron Throne Build Server",
                    from: 'The Iron Throne Build Server <jenkins@theironthrone.net>',
                    replyTo: 'noreply@theironthrone.net',
                    subject: "Throneteki-deck-helper build #${env.BUILD_NUMBER} fixed (${env.BRANCH_NAME} - ${GIT_COMMIT_HASH})",
                    to: GIT_EMAIL
                }
            }
        }
        ...
        failure {
            ...
            mail body: "Hello ${GIT_NAME},\n\nI'm sorry to report that the throneteki-deck-helper build is ${BUILD_STATUS}.  Please see details of the breakage here:\n\n${env.BUILD_URL}\n\nKind regards,\nThe Iron Throne Build Server",
            from: 'The Iron Throne Build Server <jenkins@theironthrone.net>',
            replyTo: 'noreply@theironthrone.net',
            subject: "Throneteki-deck-helper build #${env.BUILD_NUMBER} ${BUILD_STATUS} (${env.BRANCH_NAME} - ${GIT_COMMIT_HASH})",
            to: GIT_EMAIL
        }
    }
}